...
| Intent | LLM? | Reason |
|--------|------|--------|
| greeting | No | Template |
| price | No | Template |
| compatibility | Yes | Product-specific |
| installation | Yes | Technical |
| buying_intent | Yes | High value |
| spam | No | Skip |
...
| Error | Fallback | Dashboard Action |
|-------|----------|------------------|
| TikTok disconnect | Retry 3x, alert | Red status + reconnect button |
| LLM all keys exhausted | Template reply | Yellow warning + disable auto |
| OBS websocket down | Log only, no scene switch | Red status + manual scene |
| TTS timeout | Skip voice, overlay only | Yellow warning |
| Guardrail block | Skip + log | Show reason in comment |
| Budget exceeded | Stop LLM, pregen only | Red alert + end session |
